...
class UIManager:
    ...
    def draw_sidebar(self, screen, truss, sim_ctrl, current_mode, show_benchmark_hud, is_optimizing, trails_enabled, allow_profile_switching, ui_rects, calc_util_fn):
        ...
        screen.blit(self.font_body.render("5. Benchmark", True, COLOR_TEXT_MAIN), (btn_benchmark.x + 12, btn_benchmark.y + 10))

        pygame.draw.line(screen, COLOR_UI_BORDER, (10, 310), (130, 310), 1)
        screen.blit(self.font_header.render("SYSTEM STATS", True, COLOR_TEXT_MAIN), (15, 325))
        ...
        screen.blit(self.font_body.render(f"Time: {sys_time:.2f} s", True, COLOR_TEXT_MUTED), (15, 345))
        screen.blit(self.font_body.render(f"Mass: {sys_mass:.1f} kg", True, COLOR_TEXT_MUTED), (15, 360))
        screen.blit(self.font_body.render(f"Failed: {failed_count}", True, COLOR_TEXT_MUTED), (15, 375))
        screen.blit(self.font_body.render(f"Max Vel: {sys_max_vel:.2f} m/s", True, COLOR_TEXT_MUTED), (15, 390))
        screen.blit(self.font_body.render(f"Live Util: {max_util*100.0:.1f}%", True, COLOR_TEXT_MUTED), (15, 405))
        screen.blit(self.font_body.render(f"Peak Util: {peak_str}", True, COLOR_TEXT_MUTED), (15, 420))
    ...
    def draw_selection_hud(self, screen, truss, sim_ctrl, selected_node_idx, selected_beam_idx, current_mode, input_active, input_type, input_buffer, calc_util_fn):
        header_text = ""
        ...
        if is_beam_selected:
            ...
            if b_has_history or sim_ctrl.state != "EDIT":
